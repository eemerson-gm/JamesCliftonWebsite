<?php
    ...
    if(isset($web_site) && isset($category) && isset($stars) && isset($date) && isset($title) && isset($submit) && isset($id)){

        //Gets the insert sql statement from creating the blog post.
        $sql = "UPDATE table_blog_posts SET web_site = ?, web_site_2 = ?, category = ?, image = ?, stars = ?, date = ?, amazon_link = ?, booktopia_link = ?, title = ?, description = ?, blog_post = ? WHERE blog_id = ?";
        ...
        if(mysqli_stmt_prepare($stmt, $sql)){
            ...
            mysqli_stmt_bind_param($stmt, "ssssissssssi", $web_site, $web_site_2, $category, $image, $stars, $date, $amazon_link, $booktopia_link, $title, $description, $blog_post, $id);
